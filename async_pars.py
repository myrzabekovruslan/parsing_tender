        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with aiofiles.open("contracts.csv", mode="w", encoding='utf-8') as w_file:
                file_writer = aiocsv.AsyncWriter(w_file, delimiter=";",
                                         lineterminator="\r")
                await file_writer.writerow(['ID договора', 'ID Объявления', 'Номер объявления', 'Статус плана',
                        ...
                        while True:
                            try:
                                if contract.get('trdBuyNumberAnno'):
                                    lots_response = await session.get(
                                        LOTS_URL + contract.get('trdBuyNumberAnno'),
                                        params={'limit': 500, }, headers=headers, )
                                    lots_data = await lots_response.json()
                                            ...
                                            try:
                                                ...
                                                for lot_data in lots_data.get('items'):
                                                    for plan_point in lot_data.get('point_list'):
                                                        while True:
                                                            try:
                                                                plan_point_response = await session.get(
                                                                    PLAN_URL + str(plan_point), headers=headers,
                                                                )
                                                                plan_point_data = await plan_point_response.json()
